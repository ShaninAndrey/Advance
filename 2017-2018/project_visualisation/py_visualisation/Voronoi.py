...
class Context(object):
    ...
    def outSite(self, s):
        if (self.debug):
            ...
            print ("self.circle(s.x, s.y, cradius)")
        elif (self.doPrint):
            print ("s %f %f" % (s.x, s.y))
    ...
    def outEdge(self, edge):
        ...
        if (not self.triangulate):
            if self.plot:
                ...
                print ("e %d" % edge.edgenum, " %d " % sitenumL, "%d" % sitenumR)
...
def voronoi(siteList, context):
    try:
        ...
        xx = [(siteList.get_first().y, siteList.get_first())]
        hints = [siteList.get_first()]
        minpt = Site(-BIG_FLOAT, -BIG_FLOAT)

        while True:
            if not priorityQ.isEmpty():
                minpt = priorityQ.getMinPt()
                # xx.append((minpt.y, minpt))
                # hints.append(minpt)

            if (newsite and (priorityQ.isEmpty() or cmp(newsite, minpt) < 0)):
                # newsite is smallest -  this is a site event
                context.outSite(newsite)
                xx.append((newsite.y, newsite))
                hints.append(newsite)
                ...
                xx.append((lbnd.ystar, lbnd.vertex))
                hints.append(lbnd)
                llbnd = lbnd.left
                rbnd = lbnd.right
                rrbnd = rbnd.right
...
class Halfedge(object):
    ...
    def __str__(self):
        res = "Y: " + "{0:.2f}".format(self.ystar) + "\n"
        res += "Type: Circle\n"
        res += "Is valid: " + ("True" if self.pm == 1 else "False") + " \nVertex: " + repr(self.vertex)
        return res
...
class SiteList(object):
    ...
    def get_first(self):
        return self.__sites[0]

    def __str__(self):
        return str(self.__sites)
...
def computeVoronoiDiagram(points):
    ...
    context = Context()
    xx, hints = voronoi(siteList, context)
    return xx, hints, (context.vertices, context.lines, context.edges)
